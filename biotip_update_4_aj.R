...
#' testing sd_selection
#' library(Matrix)
#' setwd("~/UChicago/BioTip_Work")
#' source("biotip_update_4_aj.R")
#' counts = matrix(sample(1:100,18),2,9)
#' colnames(counts) = 1:9
#' row.names(counts) = c('loci1','loci2')
#' cli = cbind(1:9, rep(c('state1', 'state2', 'state3'), each = 3))
#' colnames(cli) = c('samples', 'group')
#' samplesL <- split(cli[, 1], f = cli[, 'group'])
#' counts.sp = Matrix(counts, sparse = TRUE)
#' test_sd_selection <- sd_selection(counts.sp, samplesL, .01)
#' test_sd_selection_d <- sd_selection(counts, samplesL, .01)
...
#' samplesL <- split(cli[, 1], f = cli[, 'group'])
#' counts.sp <- Matrix(counts, sparse = TRUE)
#' test_sd_selection <- optimize.sd_selection(counts.sp,  samplesL,  B = 3,  cutoff =0.01)
#' test_sd_selection_d <- optimize.sd_selection(counts, samplesL, B = 3, cutoff =0.01)
...
#' row.names(counts) = c('loci1', 'loci2', 'loci3')
#' CTS = c('loci1', 'loci2')
#' counts.s = Matrix(counts, sparse = TRUE)
#' randomS <- simulation_Ic_sample(counts.s,  sampleNo=3,  Ic=3.4,  genes=CTS,  B=3, 
#'                        fun='BioTIP', plot=TRUE)
#' dim(randomS)
...
#' Ic = getIc(counts.s,  samplesL,  CTS, fun='cor')
...
#' cor_tX = cor(t(X))
#' mean(abs(cor_tX[upper.tri(cor_tX, diag = FALSE)])) # 0.8517709
#' 
#' ## Same cluster, but sub Y for X
#' cor_tY = corSparse(t(Y))
#' cor_tY = Matrix(cor_tY, sparse = TRUE)
#' mean(abs(cor_tY[upper.tri(cor_tY, diag = FALSE)])) # 0.930637
#' 
#' 
...
#' 
#' 
#' ##testing cor.shrink with sparse matrix
#' cor.matrix <- cor.shrink(Y, MARGIN=1, shrink = TRUE, targe='zero')
#' dim(cor.matrix) #[1] 500 500
#' mean(upper.tri(cor.matrix, diag=FALSE))  # 0.499
#' 
#' 
...
#' 
#' 
#' ## The same as the previous block of code,
#' ## But tested on one sparse and one dense matrix
#' Z = rbind(Y, matrix(rnorm(300*10), nrow = 300))
#' dim(Z)  #800 10 
#' cor.matrix <- cor.shrink(Y, Z, MARGIN=1, shrink = TRUE, targe='zero') 
#' dim(cor.matrix)   #[1] 500 800
#' mean(upper.tri(cor.matrix, diag=FALSE))  # 0.686875
#' 
#' 
#' Y = rbind(X, matrix(rnorm(300*10), nrow = 300))
#' dim(Y)  #800 10 
...
#run demo code but make a sparse matrix to find out errors
#maybe take my old code (logmat from my old code is apparently converted from sparse matrix) 

## Initial Results
##
##
## It appears that the functions work on sparse matrices,
## but also return a sparse matrix object.
##
## It also looks like they take the same amount of storage space
## and time as the normal matrices do.
##
## You can go easily from dense to sparse matrix
## but not the other way around.
## 
## go through whole package testing all the functions with sparse matrices
## corr.test important function to watch out for
## 
